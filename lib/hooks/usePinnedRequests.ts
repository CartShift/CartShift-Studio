...
export function usePinnedRequests(_orgId?: string) {
  ...
  const pinnedIds = useMemo(() => {
    ...
    return requests.filter(req => req.pinnedBy?.includes(userId)).map(req => req.id);
  }, [requests, userId]);
  ...
  const unpinRequest = useCallback(
    async (requestId: string) => {
      if (!userId) return;
  ...
  const isPinning = useCallback((requestId: string) => {
    return globalRequestIds.has(requestId);
  }, []);
  ...
  return useMemo(
    () => ({
      ...
      loadingRequestIds: globalRequestIds, // Expose shared state
    }),
