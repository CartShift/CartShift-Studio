...
export const AnalysisResults: React.FC<AnalysisResultsProps> = ({ results, onReset, variant = 'default' }) => {
  ...
  const cardClass = isDark
    ? 'bg-[#12121a]/90 border border-white/10 backdrop-blur-xl rounded-xl'
    ...
    <div className="space-y-6 w-full max-w-7xl mx-auto">
      ...
      >
        <div className={`absolute inset-0 bg-gradient-to-r ${
          results.overallScore >= 80 ? 'from-emerald-600 to-teal-700' :
          results.overallScore >= 50 ? 'from-amber-600 to-orange-700' :
              ...
              <div className="max-w-xl">
                <h3 className="text-2xl font-bold text-white mb-2 leading-tight">
                  {results.overallScore >= 80
                    ...
                    : (t('analyzer.results.issuesFound', { count: priorityRecs.length }) || `Attention Needed: ${priorityRecs.length} Critical Issues Detected`)}
                </h3>
                <p className="text-white/90 text-base leading-relaxed">
                  {results.overallScore >= 80
                    ? (t('analyzer.results.readyToScale') || 'Your store metrics are healthy. You are ready to scale locally and globally.')
                    : (t('analyzer.results.losingSales') || 'These issues are likely effecting your SEO ranking and conversion rates. We recommend fixing them immediately.')}
                </p>
              </div>
            </div>

            <div className="flex shrink-0 w-full md:w-auto">
              <a href={getScheduleUrl()} target="_blank" rel="noopener noreferrer" className="w-full md:w-auto">
                <Button
                  size="lg"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          <div className="p-8 flex flex-col items-center justify-center text-center h-full min-h-[300px]">
            <p className={`text-sm font-semibold uppercase tracking-wider mb-6 ${isDark ? 'text-white/50' : 'text-surface-500'}`}>
              {t('analyzer.results.overallScore') || 'Overall Health Score'}
            </p>

            <div className="relative mb-6">
              <div className={`absolute inset-0 bg-primary-500/20 blur-3xl rounded-full ${isDark ? 'opacity-30' : 'opacity-0'}`} />
              <ScoreGauge score={results.overallScore} size="lg" showLabel={false} />
            </div>

            <div className="flex flex-col gap-3 w-full max-w-[200px]">
               <div className={`text-center py-2 px-4 rounded-lg border ${
                 results.overallScore >= 80 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' :
                 results.overallScore >= 50 ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' :
                 'bg-red-500/10 border-red-500/20 text-red-500'
               }`}>
                 <span className="font-bold text-lg">
                   {getStatusLabel(results.overallScore >= 80 ? 'excellent' : results.overallScore >= 50 ? 'warning' : 'critical')}
                 </span>
               </div>
               {results.platform && (
                <p className={`text-xs ${isDark ? 'text-white/30' : 'text-surface-400'}`}>
                  {t('analyzer.results.analyzedOn', { platform: results.platform }) || `Analyzed on ${results.platform}`}
                </p>
               )}
            </div>
          </div>
        ...
        >
          <div className="p-6 sm:p-8">
            <div className="flex items-center justify-between mb-8">
               <h3 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-surface-900 dark:text-white'}`}>
                <TrendingUp className="w-5 h-5 text-primary-500" />
                {t('analyzer.results.breakdown') || 'Performance Breakdown'}
              </h3>
              <span className={`text-xs px-2.5 py-1 rounded-full border ${isDark ? 'border-white/10 text-white/50 bg-white/5' : 'border-surface-200 text-surface-500 bg-surface-50'}`}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                           <span className={`block font-bold text-sm leading-tight mb-1 ${isDark ? 'text-white' : 'text-surface-900 dark:text-white'}`}>
                            {t(`analyzer.sections.${key}` as any) || section.name}
                          </span>
                           <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${
                             section.status === 'excellent' ? 'bg-emerald-500/10 text-emerald-500' :
                             section.status === 'good' ? 'bg-green-500/10 text-green-500' :
                             section.status === 'warning' ? 'bg-amber-500/10 text-amber-500' :
                      ...
                      <span className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-surface-900 dark:text-white'}`}>
                        {section.score}
                      </span>
                    </div>

                    {/* Progress Bar */}
                    <div className={`relative h-2 rounded-full overflow-hidden ${isDark ? 'bg-black/40' : 'bg-surface-200'}`}>
                      <motion.div
                        className={`absolute inset-y-0 start-0 rounded-full ${
                          section.score >= 80 ? 'bg-gradient-to-r from-emerald-500 to-emerald-400' :
                          section.score >= 60 ? 'bg-gradient-to-r from-green-500 to-green-400' :
                          section.score >= 40 ? 'bg-gradient-to-r from-amber-500 to-amber-400' :
                          'bg-gradient-to-r from-red-500 to-red-400'
                        }`}
                        initial={{ width: '0%' }}
                        animate={{ width: `${section.score}%` }}
                        transition={{ delay: 0.5 + index * 0.1, duration: 1, type: "spring", bounce: 0 }}
                      />
                    </div>
                  </motion.div>
                );
                ...
                <div>
                  <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-surface-900'}`}>
                   ...
                   <p className={`text-sm ${isDark ? 'text-white/50' : 'text-surface-500'}`}>
                    {t('analyzer.results.prioritySubtitle') || 'Focus on these high-impact items first to improve your score.'}
                  </p>
                </div>
              </div>
        ...
        <div className="lg:col-span-2 h-full">
           <motion.div
            ...
            className={`h-full flex flex-col justify-between p-8 rounded-xl relative overflow-hidden ${
               isDark
               ? 'bg-gradient-to-br from-[#1a1a25] to-[#0f0f15] border border-primary-500/30'
          ...
          >
             ...
             <div className="relative z-10">
                <h3 className={`text-2xl font-bold mb-4 ${isDark ? 'text-white' : 'text-surface-900'}`}>
                  {t('analyzer.results.dontLetIssuesHurtSales') || "Don't Let These Issues Hurt Sales"}
                </h3>
                <p className={`text-base mb-8 leading-relaxed ${isDark ? 'text-white/70' : 'text-surface-600'}`}>
                   {t('analyzer.results.expertsCanFix', { count: priorityRecs.length }) || `Our experts can fix these ${priorityRecs.length} critical errors for you. Book a free consultation to verify the plan.`}
                </p>
             </div>

             <div className="mt-auto space-y-5 relative z-10">
                 <a href={getScheduleUrl()} target="_blank" rel="noopener noreferrer" className="block w-full">
                    <Button size="lg" className="w-full text-lg py-6 bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-500 hover:to-accent-500 border-0 shadow-lg shadow-primary-500/25 group">
                ...
                <div className="flex items-center justify-center gap-4 pt-2">
                   <div className="flex -space-x-3">
                      {[1, 2, 3].map((i) => (
                        <div key={i} className={`w-8 h-8 rounded-full border-2 flex items-center justify-center overflow-hidden ${isDark ? 'border-[#12121a] bg-surface-800' : 'border-white bg-surface-200'}`}>
                           {/* Placeholder avatars since we don't have user images handy in this context, using icons or gradients */}
                           <div className={`w-full h-full bg-gradient-to-br ${i === 1 ? 'from-blue-400 to-blue-600' : i === 2 ? 'from-purple-400 to-purple-600' : 'from-pink-400 to-pink-600'}`} />
                        </div>
                      ))}
                   </div>
                   <div className="flex flex-col">
                     <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-surface-900'}`}>
                       50+ Stores
                     </span>
      ...
      <div className="text-center pt-8 border-t border-white/5">
         <p className={`text-sm mb-4 ${isDark ? 'text-white/40' : 'text-surface-500'}`}>
           {t('analyzer.results.fullReportSent') || 'Full detailed report has been sent to your email.'}
         </p>
         <Button variant="outline" size="sm" onClick={onReset} className="border-white/10 bg-white/5 text-white/60 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all">
